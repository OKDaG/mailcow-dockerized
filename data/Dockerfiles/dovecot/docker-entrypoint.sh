...
if [[ ! -z ${REDIS_SLAVEOF_IP} ]]; then
  ...
  echo -n "redis:host=${REDIS_IP}:port=6379" > /etc/dovecot/last_login
fi
...
driver = mysql
connect = "host=mysql dbname=${DBNAME} user=${DBUSER} password=${DBPASS}"
user_query = SELECT CONCAT(JSON_UNQUOTE(JSON_EXTRACT(attributes, '$.mailbox_format')), mailbox_path_prefix, '%d/%n/${MAILDIR_SUB}:VOLATILEDIR=/var/volatile/%u') AS mail, 5000 AS uid, 5000 AS gid, concat('*:bytes=', quota) AS quota_rule FROM mailbox WHERE username = '%u' AND (active = '1' OR active = '2')
...
passdb {
  driver = static
  args = allow_real_nets=${SOGO_IP}/24 password={plain}${RAND_PASS}
}
EOF
else
...
IMAPSYNC_TABLE=$(mysql --host=mysql -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'imapsync'" -Bs)
[[ ! -z ${IMAPSYNC_TABLE} ]] && mysql --host=mysql -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "UPDATE imapsync SET is_running='0'"
...
while [[ ${VERSIONS_OK} != 'OK' ]]; do
  if [[ ! -z $(mysql --host=mysql -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = \"${DBNAME}\" AND TABLE_NAME = 'versions'") ]]; then
    VERSIONS_OK=OK
  else
